...
function MediaEngine:setVolume(pct)
    ...
    elseif self.backend == self.BACKENDS.MPLAYER then
        ...
        pcall(function()
            ...
            if DeviceMod and DeviceMod.isKindle and DeviceMod:isKindle() then
                os.execute(string.format("lipc-set-prop com.lab126.audiomgrd volume %d >/dev/null 2>&1", math.floor(pct / 5)))
                set_hw = true
                return
            end
        end)

        if not set_hw and not self.is_paused then
            -- Fallback for raw pipe decode: restart at the audible position
